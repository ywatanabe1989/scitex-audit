"""`scitex-audit skills` — list / get / install agent-facing skills.

Ported from scitex-security 0.1.4 (absorbed into scitex-audit per
ADR-0001 in scitex-dev #139). Self-contained — no scitex-dev runtime
dep — walks the package's own `_skills/scitex-audit/` directory
directly.
"""
...
def _skills_root() -> Path:
    """Resolve the bundled `_skills/scitex-audit/` directory."""
    import scitex_audit

    pkg_dir = Path(scitex_audit.__file__).parent
    return pkg_dir / "_skills" / PKG


def _list_skill_files(root: Path) -> list[Path]:
    """All `.md` files under `_skills/scitex-audit/` (recursive), excluding SKILL.md."""
    if not root.is_dir():
        return []
...
def skills_group(ctx) -> None:
    """Agent-facing skills bundled with scitex-audit.

    \b
    Examples:
      $ scitex-audit skills list
      $ scitex-audit skills get 01_installation
      $ scitex-audit skills install                  # → ~/.scitex/dev/skills/scitex-audit/
      $ scitex-audit skills install --claude-symlink # also expose to ~/.claude/skills/scitex/
    """
...
def skills_get(name: str, as_json: bool) -> None:
    ...
    Example:
      $ scitex-audit skills get 01_installation
      $ scitex-audit skills get 02_quick-start --json
    """
    root = _skills_root()
...
@click.option(
    ...
    type=click.Path(),
    default=None,
    help="Destination dir (default: ~/.scitex/dev/skills/scitex-audit/).",
)
@click.option(
    "--no-link",
    "no_link",
...
def skills_install(
    ...
    \b
    Default: symlink the entire `_skills/scitex-audit/` dir to
    ~/.scitex/dev/skills/scitex-audit/ so add/rename/delete in
    source propagates immediately.

    \b
    Example:
      $ scitex-audit skills install
      $ scitex-audit skills install --claude-symlink
      $ scitex-audit skills install --no-link --dest /tmp/scitex-audit-skills
    """
